...
对于用到 CNN 模型的那条路径：`gen_scene.py` 在大图里生成小尺寸的码，
`run_cpp_nn.py` 产出参考输出（它需要 venv4 和上游的 Caffe 模型，
[wxscan-weights](https://github.com/wilinz/wxscan-weights) 里的
`tools/convert.py download` 会把它们下载下来；如果那个 checkout 不在本仓库旁边，
把它的目录作为第三个参数传进去），Rust 那一侧则运行
...
| 语料 | 一致程度 | 说明 |
|---|---|---|
| 160 张图，不带模型 | 文本 159/160 | 唯一的差异是一张反色图片，Rust 移植解出来了而 C++ 实现没有；解出的文本与真值一致。角点在 152/154 上逐位相同，另有 2 张是亚像素级差异。 |
| 24 张场景图，带模型 | 文本 24/24 | 角点最大差异 2.9 px，来自 SSD 框的轻微偏移，因为 TFLite 和 Caffe 的浮点累加顺序不同。 |

那些亚像素差异源自 `cv::adaptiveThreshold` 内部的高斯模糊：OpenCV 对 8U 图像使用
定点实现，而这个移植在 f32 上累加（见 cvlite 仓库里 `src/threshold.rs` 的注释）。
